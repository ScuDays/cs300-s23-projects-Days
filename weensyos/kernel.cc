...
void kernel(const char* command) {
  ...
  for (vmiter it(kernel_pagetable); it.va() < MEMSIZE_PHYSICAL;
       it += PAGESIZE) {
    /** Modified code*/
    if (it.va() != 0) {
      //对于内核的页面，用户不应该具有访问权限
...
void kfree(void* kptr) {
  ...
  uintptr_t free_ptr = (uintptr_t)kptr;
  if (pages[free_ptr / PAGESIZE].used() &&
      allocatable_physical_address(free_ptr)) {
    pages[free_ptr / PAGESIZE].refcount--;
  }
  if (free_ptr < next_alloc_pa) {
    next_alloc_pa = free_ptr;
  }
}
...
void process_setup(pid_t pid, const char* program_name) {
  ...
  //     permissions in the destination page table vmiter(ptable[pid].pagetable,
  //     it.va()).map(pa, perm);
  // }

  for (vmiter kernel_It(kernel_pagetable, 0), process_It(proc_pagetable, 0);
       kernel_It.va() < PROC_START_ADDR;
       kernel_It += PAGESIZE, process_It += PAGESIZE) {
    int PTEP = 0;
    int PTEW = 0;
    int PTEU = 0;
    ...
    if (PTEP == 1 && PTEW == 0 && PTEU == 0) {
      ...
      // log_printf("this is pid %d,VA %p maps to PA %p with PERMS %p, %p,
  ...
  // Using the loader, we're going to start loading segments of the program
  // binary into memory (recall that an executable has code/text segment, data
  // segment, etc).
  ...
  for (loader.reset(); loader.present(); ++loader) {
    for (uintptr_t a = round_down(loader.va(), PAGESIZE);
         a < loader.va() + loader.size(); a += PAGESIZE) {
      // `a` is the virtual address of the current segment's page.
      vmiter it(ptable[pid].pagetable, a);

      // assert(!pages[a / PAGESIZE].used());

      // Read the description on the `pages` array if you're confused about what
      // it is. Here, we're directly getting the page that has the same physical
      // address as the virtual address `a`, and claiming that page by
      // incrementing its reference count (you will have to change this later).
  ...
  for (loader.reset(); loader.present(); ++loader) {
    memset((void*)vmiter(ptable[pid].pagetable, loader.va()).pa(), 0,
           loader.size());
    memcpy((void*)vmiter(ptable[pid].pagetable, loader.va()).pa(),
           loader.data(), loader.data_size());
  }
  ...
  // Again, we're using the physical page that has the same address as the
  // `stack_addr` to maintain the one-to-one mapping between physical and
  // virtual memory (you will have to change this later).
...
void exception(regstate* regs) {
  ...
  // Show the current cursor location and memory state (unless this is a kernel
  // fault).
  ...
  switch (regs->reg_intno) {
    ...
    case INT_PF: {
      ...
      const char* operation =
          regs->reg_errcode & PFERR_WRITE ? "write" : "read";
...
//    have to change this).
int syscall_page_alloc(uintptr_t addr) {
  // Check if the address is within the range of the application region of
  // memory.
...
pid_t syscall_fork() {
  ...
  if ((void*)process_pagetable == (void*)nullptr) {
    //syscall_exit_assist(current->pid);
    return -1;
  }
  ...
  for (vmiter it(current->pagetable, 0), process_it(process_pagetable, 0);
  it.va() < MEMSIZE_VIRTUAL; 
  it += PAGESIZE, process_it += PAGESIZE) {
    bool user_access = it.user();
    uint64_t this_perm = it.perm();
    if (it.present() && !it.writable()) {
        if (vmiter(ptable[child_pid].pagetable, it.va()).try_map(it.pa(), this_perm) == -1) {
                syscall_exit_assist(child_pid);
                return -1;
            }
      ...
      if (it.va() == CONSOLE_ADDR) {
        // process_it.try_map(CONSOLE_ADDR, this_perm);
           if (vmiter(ptable[child_pid].pagetable, it.va()).try_map(CONSOLE_ADDR, this_perm) == -1) {
                syscall_exit_assist(child_pid);
                return -1;
            }
      } 
        ...
        memcpy((void*)child_pa, (void*)it.pa(), PAGESIZE);
         if (vmiter(ptable[child_pid].pagetable, it.va()).try_map(child_pa, this_perm) == -1) {
                syscall_exit_assist(child_pid);
                kfree((void*) child_pa);
                return -1;
            }
...
void syscall_exit_assist(pid_t release_pid) {
  x86_64_pagetable* release_table = ptable[release_pid].pagetable;
  for (vmiter it(release_table, PROC_START_ADDR); it.va() < MEMSIZE_VIRTUAL;
       it += PAGESIZE) {
    if (it.user() && it.va() != CONSOLE_ADDR) {
      kfree((void*)it.pa());
    }
  }
  for (ptiter it(release_table); it.active(); it.next()) {
    kfree(it.kptr());
  }
  kfree(release_table);
  ptable[release_pid].state = P_FREE;
  // Implement for Step 7!
  // panic("Unexpected system call %ld!\n", SYSCALL_EXIT);
}
void syscall_exit() { syscall_exit_assist(current->pid); }
